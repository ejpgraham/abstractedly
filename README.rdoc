...
Mining the keywords was reasonably straightforward. The RSS feed lists a URL for each abstract, which helpfully points to a website where keywords are displayed. I used the Mechanize Gem to direct a GET request to each URL. Given the large number of abstracts, this is a time consuming process! However, it occurs in the background of the application and does not interfere with the user experience.
 ...
 Some Journals do not provide keywords at all, even on the abstract's page, but thankfully these are few and far enough between that I was willing to accept that a few abstracts wouldn't have keywords.
...
The User expects a simple, clean interface as the abstracts are the star of the show. The subscription manager page allows the user to select which journal feeds they interested in. Once a user is 'subscribed' to a journal feed, its associated journal issues and abstracts are displayed to the user.

Goals For the Future:

Updating to Rails 5 -

Early on in Abstractedly's development, I noticed I hadn't actually started my app in the latest version of rails - it was 4 instead of 5.
